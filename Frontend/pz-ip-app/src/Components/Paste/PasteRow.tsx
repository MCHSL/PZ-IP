...
const PasteRow = ({ id, title, author, createdAt, updatedAt, isPrivate, refetch }: Props) =>
{
	...
	return (
		<>
			...
			<tr>
				<td ><Button variant="link" onClick={() => navigate(`/paste/${id}`, { state: { returnTo: location.pathname } })}>{title}</Button></td>
				...
				{(user && (user?.isStaff || user.id === author.id)) ? <td><Button variant="link" onClick={() => { setDeleteVisible(true) }}><FontAwesomeIcon icon={solid('trash-can')} /></Button></td> : <td></td>}
			</tr>
		</>
	);
};
