...
const PaginatingList = ({ children, visible, totalItems, page, setPage, itemsPerPage, setItemsPerPage }: Props) =>
{
		...
		<>
			...
			<div style={{ margin: "auto" }}>
				<Pagination style={{ margin: "auto", justifyContent: "center" }}>
					...
					<Pagination.Next disabled={page === (last_page - 1)} onClick={() => setPage(page + 1)} />
					<Pagination.Last disabled={page === (last_page - 1)} onClick={() => setPage(last_page - 1)} />
				</Pagination>
				<DropdownButton style={{ display: "flex", width: "50%", justifyContent: "center", margin: "auto", marginTop: "10px", marginBottom: "10px" }} title={`${itemsPerPage} na stronę`} onSelect={(e) => setItemsPerPage(Number(e))}>
					{
						[5, 10, 15, 25, 50, 100].map((num) =>
						{
							return (<Dropdown.Item key={num} eventKey={num} active={num === itemsPerPage}>{num}</Dropdown.Item>)
						})
					}
				</DropdownButton>
			</div>
